...
{
	pr_info("IRQ %d triggered, usbin %d target_ma%d\n", irq,
		is_usb_chg_plugged_in(the_chip), usb_target_ma);
	if (usb_target_ma)
...
static void unplug_check_worker(struct work_struct *work)
{
	...
	if (chg_gone == 1  && active_chg_plugged_in == 1) {
		pr_info("chg_gone=%d, active_chg_plugged_in = %d\n",
